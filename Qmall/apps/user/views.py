...
class UserNameCheck(View):
    def get(self, username):
        if re.match(r'^[a-zA-Z0-9_-]{5,20}$', username) and re.match('^[0-9]+$', username):
        ...
        if if_user_exist:
            ...
            return JsonResponse({"code": 0, "count": 0, "message": "Success!"})
...
class EmailCheck(View):
    def get(self, email):
        ...
        if if_user_exist:
            return JsonResponse({"code": 0, "count": 1, "message": "Email is existed!"})
        else:
            return JsonResponse({"code": 0, "count": 0, "message": "Success!"})
